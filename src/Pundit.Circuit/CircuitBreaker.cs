...
namespace Pundit.Circuit
{
    public class CircuitBreaker : ICircuitBreaker, ICircuitBreakerSwitch
    {
        ...
        public double ServiceLevel { get { return _currentState.ServiceLevel; } }
        ...
        public virtual async Task ExecuteAsync(Func<Task> func, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (func == null)
                throw new ArgumentNullException("Function", "Function cannot be null.");

            await _currentState.InvokeAsync(func, cancellationToken);
        }

        public virtual async Task<T> ExecuteAsync<T>(Func<Task<T>> func, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (func == null)
                throw new ArgumentNullException("Function", "Function cannot be null.");

            return await _currentState.InvokeAsync(func, cancellationToken);
        }
